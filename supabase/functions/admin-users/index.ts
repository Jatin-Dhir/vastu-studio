    ...
    switch (body.action) {
      case 'create': {
        ...
        const { data, error } = await admin.auth.admin.createUser({
          phone, password, phone_confirm: true, user_metadata: { name },
        })
        if (error) return json({ error: error.message }, 400)
        const id = data.user.id
        const expires = body.expires_at ? String(body.expires_at) : null
        // the trigger created the profile row; fill in what the admin set
        const { error: pErr } = await admin.from('profiles')
          .update({ name, expires_at: expires, notes: body.notes ? String(body.notes) : null })
          .eq('id', id)
