...
interface Props {
  ...
  exclude?: MarkerKind[]
  /** how many recently-used kinds ride along as one-click pills */
  recents?: number
}
 ...
 * recent pills, everything else is a couple of keystrokes.
 */
export function KindPicker({ value, onChange, exclude = [], recents = 4 }: Props) {
  ...
  const pills = recentKinds.filter((k) => allowed(k) && k !== value).slice(0, recents)
  ...
  const pick = (k: MarkerKind, viaKeyboard = false) => { onChange(k); close(viaKeyboard) }
          ...
          <div className="kind-pop">
            <label className="kind-search">
              ...
              <input ref={inputRef} value={q} placeholder="Search rooms & objects" aria-label="Search types"
                role="combobox" aria-expanded aria-controls="kind-picker-list" aria-autocomplete="list"
                aria-activedescendant={items[hl] ? `kind-opt-${items[hl]}-${hl}` : undefined}
            ...
            <div className="kind-list" ref={listRef} role="listbox" id="kind-picker-list">
              ...
              {rows.map((r) => r.type === 'head'
                ...
                : (() => {
                  ...
                  return (
                    <button key={`${r.kind}-${r.idx}`} id={`kind-opt-${r.kind}-${r.idx}`} type="button" role="option"
